...
from fcnn_util import plot_roc, store_report
...
def test(x_data, y_data, step):
    ...
    with tf.Graph().as_default() as g:
        ...
        with tf.Session() as sess:
            ckpt = tf.train.get_checkpoint_state(fcnn_backward.MODEL_SAVE_PATH)
            if ckpt and ckpt.model_checkpoint_path:
                saver.restore(sess, ckpt.model_checkpoint_path)
                global_step = ckpt.model_checkpoint_path.split('/')[-1].split('-')[-1]

                xs = np.reshape(x_data, (x_data.shape[0], fcnn_forward.INPUT_NODE))
                ys = y_data
                        
                accuracy_score = sess.run(accuracy, feed_dict={x: xs, y_: ys})
                result = accuracy_score

                if step == fcnn_backward.STEPS:
...
def main(step):
    gray_data = np.load(fcnn_backward.DATA_PATH)
