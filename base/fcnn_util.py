...
def plot_roc(y_data, y_pred):
    ...
    plt.savefig('images/micro_macro_' + fcnn_backward.param_to_change_and_value + '.png')
    plt.clf()
    for i in range(N_CLASSES):
    ...
    plt.title('Some extension of Receiver operating characteristic to multi-class')
    plt.legend(loc="lower right")
    plt.savefig('images/every_class_' + fcnn_backward.param_to_change_and_value + '.png')

def store_report(y_data, y_pred):
    if 'report_files' not in os.listdir():
        os.makedirs('report_files')
    report_file = open('report_files/' + fcnn_backward.param_to_change_and_value + '_' + 'report.txt', 'a+')
...
def store_accuracy(step, accuracy):
    ...
    accuracy_file = open('accuracy_files/' + fcnn_backward.param_to_change_and_value + '.txt', 'a+')
