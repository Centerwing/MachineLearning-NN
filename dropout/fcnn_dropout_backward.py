...
import fcnn_dropout_test
from fcnn_dropout_util import store_accuracy
...
param_to_change_and_value = 'Dropout'


def backward(x_data, y_data):
    x_data = np.reshape(x_data, (x_data.shape[0], fcnn_dropout_forward.INPUT_NODE))/255 # 拉直x
    y_data = np.eye(y_data.shape[0], fcnn_dropout_forward.OUTPUT_NODE)[y_data].tolist()  # one_hot y

    x = tf.placeholder(tf.float32, [None, fcnn_dropout_forward.INPUT_NODE])
    y_ = tf.placeholder(tf.float32, [None, fcnn_dropout_forward.OUTPUT_NODE])
    y = fcnn_dropout_forward.forward(x, REGULARIZER, True)
    ...
    with tf.Session() as sess:
        ...
        for i in range(STEPS):
            ...
            if (i+1) % 200 == 0:
                ...
                accuracy = fcnn_dropout_test.main(step)
